...
class SearchService:
    ...
    async def search(self,
                    ...
                    transaction_token: Optional[str] = None) -> SearchResponse:
        """Search documents using the model.
        ...
        options = None
        if any([limit, similarity_threshold, include_metadata, include_embeddings]):
        ...
        try:
            ...
            logger.error(f"Search request failed for model {self.model_info.name}: {e}")
            raise
    ...
    async def search_with_context(self,
        ...
        """Search with additional context about data sources.
        
        Args:
            query: Search query string
            context_sources: List of context sources to consider
            **kwargs: Additional search arguments
            
        Returns:
            Search response with context-aware results
        """
        # Enhanced query with context
        if context_sources:
            context_str = ", ".join(context_sources)
            enhanced_query = f"{query} (sources: {context_str})"
        else:
            enhanced_query = query
        
        return await self.search(enhanced_query, **kwargs)
    
    def _build_rpc_payload(self, request: SearchRequest) -> Dict[str, Any]:
        """Build RPC payload from search request.
        ...
        if request.options:
            ...
            if request.options.limit is not None:
                options["limit"] = request.options.limit
            if request.options.similarity_threshold is not None:
                options["similarityThreshold"] = request.options.similarity_threshold
            if request.options.include_metadata is not None:
                options["includeMetadata"] = request.options.include_metadata
            if request.options.include_embeddings is not None:
                options["includeEmbeddings"] = request.options.include_embeddings
            
            if options:
                payload["options"] = options
        else:
            # Default options
            payload["options"] = {"limit": 3}
